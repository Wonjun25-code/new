...
model = Prophet()
model.add_seasonality(name='sunspot_11yr', period=11, fourier_order=5)
model.fit(df)
...
ax.plot(df["ds"], df["y"], marker='o', color='blue', label='Actual')
...
merged = pd.merge(forecast, df, on="ds", how="inner")
